...
const renderLogin = async(req,res) =>{ 
  ...
  const postdata = async(req,res) => {
    try {
        ...
        var month = today.getMonth() + 1; // Tháng trong JavaScript bắt đầu từ 0, nên cần cộng thêm 1
        ...
        await getAddressFromCoordinates(parseFloat(tmp.coordinatesDevice.latitude), parseFloat(tmp.coordinatesDevice.longtitude)).then(async (diachi) => {
           tmp.diachi = diachi;
           tmp.date.ngaythangnam = fullDate;
           tmp.date.giophutgiay = time;
        });
        ...
        await Product.updateMany({ iddevice: product.iddevice }, { distance : caldistance.toFixed(2)*1000 });
